...
function compose_letter($str) {
	if (issetstr($str)) {
		...
		$letter .= "<p>Please confirm that you want to use this email address for your Cupid's Arrow account. Once it's done you will be able to start using this service.</p>";
		...
		$letter .= $str . '<br />';
		$letter .= '<br /><p align="center">&copy Cupid | 2019</p>';
		return $letter;
	}
}
if (!isset($_SESSION['email'])) {
	ft_redirectuser('../signup.php?error_10=1' . $url);
}
$date= date_create();
...
try {
	...
	email_client($e, "Email verification | Cupid", compose_letter($url));
